...
- Link the framework's precompiled ESP32-S3 `cam_hal` and `ll_cam` unchanged. The known-good CoreS3 example uses these stock objects; source-building custom DMA limits, EOF queue depth, task affinity, or clock/lifecycle patches produced about 66 EOF interrupts per VSYNC and no completed frame. Override only the GC0308 sensor unit and leave its PCLK register at the framework default.
